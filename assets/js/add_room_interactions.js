...
//		IF WE HAVE USER WITH ROOM ALREADY, AND USER IS VIEWING OR EDITING ROOM => ROOM HAS ALL DETAILS SELECTED
			...
			if ( (
			     sessionStorage.getItem ( 'room_to_edit' ) !== 'undefined' && sessionStorage.getItem ( 'room_to_edit' )
			     !== null )
...
//				IF USER SELECTS ANYTHING FROM THE SELECTION EXCEPT board, WE WILL GIVE HIM GREEN LIGHT TO CONTINUE
						...
						/// IMAGE CARD FOOTER CHANGED TO .bg_green text-light, EXCEPT IF IT IS BOARD, BECAUSE USER
						// NEEDS
						// TO ADD PRICE FOR THE BOARD, .bg_orange IS APPLIED FIRST, AND WHEN HE ADDS PRICE , IT WILL
...
//					AND CURRENTLY SELECTED OPTION WILL GET bg_green text-light CLASS
						...
						// SELECTED-DESELECTED, NEEDED LATER IF USER DESELECTS ALL OF THEM , TO RETURN COLOR TO
						// ORIGINAL STATE
...
//let unique = [ ...new Set ( all ) ];  TO GET UNIQUE ARRAY
		...
		if ( new_auto_c ) localStorage.setItem (
